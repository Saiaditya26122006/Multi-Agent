...
class Challenge(BaseModel):
    ...
    challenge_type: Literal["logical_gap", "overconfidence", "unsupported", "math_error", "contradiction", "survivorship_bias"] = Field(...)
...
class DevilsAdvocateOutput(BaseModel):
    task_id: str
    section_number: str
    verdict: Literal["pass", "revise", "reject"] = Field(..., description="Overall verdict on section quality")
    challenges: List[Challenge] = Field(..., description="List of challenges found")
    confidence_assessment: Literal["honest", "inflated", "deflated"] = Field(..., description="Is the section's self-reported confidence accurate?")
    recommended_confidence: Literal["high", "medium", "low"] = Field(..., description="What confidence should actually be")
    assumptions_grade: Literal["well_sourced", "mixed", "mostly_unsupported"] = Field(...)
    overall_reasoning_quality: Literal["strong", "adequate", "weak"] = Field(...)
