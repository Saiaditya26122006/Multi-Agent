#!/usr/bin/env python3
"""Measure Feed classifier v2 (flat retrieval) against the answer key.

Runs every fact in evaluation/classifier_answer_key_draft.csv through the
flat-retrieval classifier and reports the six breakdowns:
...
from services.feed_classifier_v2 import (  # noqa: E402
    AUTO_FILE,
    PARENT_PARKED,
    REVIEW,
    classify_batch,
...
def main() -> None:
    ...
    print("loading leaf ids ...")
    leaf_ids = load_leaf_ids(sb)
    print(f"classifying {len(rows)} facts (flat HNSW retrieval) ...\n")
    routings = classify_batch([r["fact_text"] for r in rows], sb, leaf_ids)
    ...
    for row, rt in zip(rows, routings):
        ...
        rec = {
            ...
            "leaf_hit": rt.top_id in targets,
            "domain_hit": (domain_of(rt.top_id) if rt.top_id else None) in {domain_of(t) for t in targets},
            "section_hit": (section_of(rt.top_id) if rt.top_id else None) in {section_of(t) for t in targets},
        }
        recs.append(rec)
    ...
    print("=" * 78)
    print("FEED CLASSIFIER v2 — flat HNSW retrieval over leaves, no LLM judge")
    ...
    for r in wrong:
        print(f"       - {r['row']['correct_node_id']} -> got {r['rt'].top_id} "
              f"({r['rt'].decision}, leaf conf {r['rt'].top_confidence})")
    ...
    for r in deg:
        ...
        print(f"     {flag}{r['row']['correct_node_id']:<12} decision={rt.decision:<14} "
              f"best_leaf={rt.top_id} degraded={rt.leaf_degraded} "
    ...
    for lo, hi in BANDS:
        sub = [r for r in primary
               if r["rt"].top_confidence is not None
               and lo <= r["rt"].top_confidence < hi]
        if not sub:
            continue
    ...
    cum = sorted(primary, key=lambda r: -(r["rt"].top_confidence or 0))
    ...
    for t in (0.75, 0.7, 0.65, 0.6, 0.55, 0.5, 0.45, 0.4):
        sel = [r for r in cum if (r["rt"].top_confidence or 0) >= t]
        if not sel:
            continue
